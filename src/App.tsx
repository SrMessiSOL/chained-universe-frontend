...
import {
  GameClient,
  type BurnerRecoveryPromptRequest,
        ...
        <div className="modal-section">
          {lines.map((line, index) => (
            <div
              ...
              style={{
                fontSize: "11px",
                color: line.startsWith("-") ? "var(--text)" : "var(--dim)",
                lineHeight: 1.6,
                marginBottom: 10,
                whiteSpace: "pre-wrap",
              }}
...
interface RecoveryPasswordModalState {
  mode: "create" | "unlock";
  wallet: string;
  planet: string;
}
...
interface RecoveryPasswordModalProps {
  request: RecoveryPasswordModalState | null;
  busy?: boolean;
  error?: string | null;
  onCancel: () => void;
...
const RecoveryPasswordModal: React.FC<RecoveryPasswordModalProps> = ({
  ...
  useEffect(() => {
    setPassword("");
    setConfirmPassword("");
  }, [request?.mode, request?.planet, request?.wallet]);
    ...
    >
      <div className="modal" style={{ maxWidth: "560px" }}>
        <div className="modal-title" style={{ color: "var(--warn)" }}>
          {isCreate ? "SECURE GAME RECOVERY" : "RESTORE GAME ACCESS"}
        </div>

        <div className="modal-section">
          <div style={{ fontSize: "11px", color: "var(--text)", lineHeight: 1.7, marginBottom: 14 }}>
            {isCreate
              ? "Create a recovery password to protect your delegated game access. You will need the same wallet and this password to restore the game safely on another device."
              : "Enter your recovery password to restore delegated gameplay on this device. The same wallet and the same recovery password are required."}
          </div>
          <div style={{
            padding: "12px 14px",
            ...
            If this password is lost, there is a high chance you will lose access to your delegated game data on another device.
          </div>
        </div>
        ...
        <div className="modal-section">
          ...
          <input
            ...
            placeholder={isCreate ? "Choose a strong recovery password" : "Enter your recovery password"}
            disabled={busy}
          />
        </div>
        ...
        {isCreate && (
          <div className="modal-section">
            ...
            <input
              ...
              disabled={busy}
            />
            <div style={{ fontSize: "10px", color: "var(--dim)", marginTop: 8 }}>
              Use at least 8 characters and store it somewhere safe.
            </div>
          </div>
        )}
        ...
        {!isCreate && (
          <div className="modal-section">
            <div style={{ fontSize: "10px", color: "var(--dim)", lineHeight: 1.6 }}>
              Same device recovery will be cached locally in encrypted form to reduce repeated prompts after refresh.
            </div>
          </div>
        )}

        {error && (
          <div style={{ color: "var(--danger)", fontSize: "10px", marginTop: -6, marginBottom: 8 }}>
            {error}
          </div>
        )}
        ...
        <div className="modal-footer">
          ...
          >
            {isCreate ? "SAVE RECOVERY PASSWORD" : "RESTORE GAME"}
          </button>
        </div>
      </div>
    </div>
...
const App: React.FC = () => {
  ...
  const [createProgress, setCreateProgress] = useState("Signing with wallet: creating player profile");
  const [txBusy, setTxBusy] = useState(false);
  const [txProgress, setTxProgress] = useState("Processing on-chain transaction...");
  ...
  const [recoveryPrompt, setRecoveryPrompt] = useState<RecoveryPasswordModalState | null>(null);
  const [recoveryPromptError, setRecoveryPromptError] = useState<string | null>(null);
  const [recoveryPromptBusy, setRecoveryPromptBusy] = useState(false);

  const clientRef = useRef<GameClient | null>(null);
  const selectedPlanetPdaRef = useRef<string | null>(null);
  const refreshPromiseRef = useRef<Promise<PlayerState[] | void> | null>(null);
...
const requestBurnerRecoveryPassphrase = useCallback((request: BurnerRecoveryPromptRequest) => {
  setRecoveryPromptBusy(false);
...
const handleRecoveryPromptCancel = useCallback(() => {
  if (recoveryPromptBusy) return;

  recoveryPromptResolverRef.current?.reject(new Error("User cancelled recovery password entry"));
  recoveryPromptResolverRef.current = null;
  setRecoveryPrompt(null);
  setRecoveryPromptError(null);
  setRecoveryPromptBusy(false);
  clientRef.current?.clearCachedBurnerRecoveryPassphrase();
}, [recoveryPromptBusy]);

const handleRecoveryPromptSubmit = useCallback((password: string) => {
  if (recoveryPrompt?.mode === "create" && password.length < 8) {
    setRecoveryPromptError("Recovery password must be at least 8 characters.");
    return;
  }

  setRecoveryPromptBusy(true);

  recoveryPromptResolverRef.current?.resolve(password);
  recoveryPromptResolverRef.current = null;
  setRecoveryPrompt(null);
  setRecoveryPromptError(null);
  setRecoveryPromptBusy(false);
...
clientRef.current = new GameClient(connection, provider, {
  requestBurnerRecoveryPassphrase,
});
...
}, [connected, anchorWallet, publicKey, connection, loadAllPlanets, requestBurnerRecoveryPassphrase]);
  ...
  const refreshPromise = (async () => {
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("Planet refresh timed out.")), 7000);
      });
      await Promise.race([loadAllPlanets(publicKey), timeoutPromise]);
    } catch (e) {
      throw e;
    } finally {
...
const withTx = async (label: string, fn: () => Promise<string>) => {
  ...
  try {
    ...
    try {
      ...
      setError((prev) => prev ?? (refreshErr?.message ?? "Planet refresh timed out."));
    }
    setTimeout(() => {
      void refresh().catch(() => {});
    }, 1500);
    ...
    setTxBusy(false);           // ← add this
    setTxProgress("Processing on-chain transaction...");
  }
};

  const handleRedelegatePlanet = () => {
    if (!clientRef.current || !state) return;
    void withTx("Re-delegate planet", () =>
      clientRef.current!.delegatePlanet(new PublicKey(state.entityPda), setTxProgress)
    );
  };

  const handleUndelegatePlanet = () => {
    if (!clientRef.current || !state) return;
    void withTx("Undelegate planet", () =>
      clientRef.current!.undelegatePlanet(new PublicKey(state.entityPda))
    );
  };
 ...
 const createPlanet = async () => {
  ...
  setError(null);
  setCreating(true);
  setCreateProgress("Signing with wallet: creating player profile");
  try {
    ...
    setCreating(false);         // ← this was already here, good
    setCreateProgress("Signing with wallet: creating player profile");
  }
};
  ...
  const handleResolveColonize = async (mission: Mission, slotIdx: number) => {
    if (!clientRef.current || !state) return;
    setConfirmation({ kind: "resolveColonize", mission, slotIdx });
  };
  ...
  const executeResolveColonize = async (mission: Mission, slotIdx: number) => {
    ...
    setTxBusy(true);
    setTxProgress("Signing with wallet: creating colony entity and components");
    setError(null);
    let resolved = false;
    try {
      ...
      setError(e?.message ?? "Resolve colonize failed");
    } finally {
      setTxProgress("Processing on-chain transaction...");
    }
  ...
  const confirmationTitle =
    confirmation?.kind === "resolveColonize"
      ? "RESOLVE COLONIZE"
      : "";
  ...
  const confirmationLines =
    confirmation?.kind === "resolveColonize"
      ? [
          "Resolving a colony still needs wallet signatures even in always-on ER mode.",
          "- Wallet signatures: create the new colony account and then resolve the colonize mission",
          "- Session signatures: initialize the colony and resolve the colonize mission",
          "This keeps the new planet immediately playable after it is created.",
        ]
      : [];
  ...
  return (
    <>
      ...
      <Starfield />
<LoadingOverlay
  visible={(txBusy || creating) && !recoveryPrompt}
  message={creating ? createProgress : txProgress}
/>      
      {!connected && (
...
<nav className="nav" style={{ 
  ...
  />
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          if (confirmation?.kind === "resolveColonize") {
            void executeResolveColonize(confirmation.mission, confirmation.slotIdx);
          }
        }}
...
<RecoveryPasswordModal
  ...
  busy={recoveryPromptBusy}
  error={recoveryPromptError}
  onCancel={handleRecoveryPromptCancel}
  onSubmit={handleRecoveryPromptSubmit}
/>
    </>
  );
};
