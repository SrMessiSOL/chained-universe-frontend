...
const RESOURCES_IDL: Idl = {
  ...
  instructions: [],
  accounts: [{ name: "Resources", discriminator: [252,239,111,79,54,7,67,233] }],
  types: [{
    name: "Resources",
    type: {
      kind: "struct" as const,
...
export class SolarGridClient {
  ...
  private planetProgram: any;
  private resourcesProgram: any;
  private fleetProgram: any;
  ...
  constructor(connection: Connection, provider: AnchorProvider) {
    ...
    (AnchorProvider as any).local = () => provider;
  ...
  async initializeWorld(): Promise<GameAddresses> {
    ...
    if (!SHARED_WORLD_PDA) {
      throw new Error("Shared universe is not configured. Set VITE_SHARED_WORLD_PDA in frontend env.");
    }
    const worldPda = new PublicKey(SHARED_WORLD_PDA);

    // 1. Create player entity inside shared world
  ...
  async launchFleet(params: {
    ...
    const worldPda = params.worldPda;
